...
test.describe('Listing Flyer — exports', () => {
  ...
  test('exports a valid JPEG with matching visual snapshot', async ({
    ...
    // Visual snapshot on the JPEG. toMatchSnapshot decodes both sides as
    // PNG for pixel diff, so we can't pass the raw JPEG bytes directly
    // (Playwright errors with "Could not decode expected image as PNG").
    // Render the JPEG into Chromium via setContent + <img>, then screenshot
    // the image element — that screenshot is a PNG buffer that diffs
    // cleanly. setContent navigates away from /listing-flyer but the
    // export already produced its file, so page state doesn't matter past
    // this point.
    ...
    const pngScreenshot = await img.screenshot({ type: 'png' });
    expect(pngScreenshot).toMatchSnapshot('listing-flyer-jpeg.png', {
      threshold: 0.2,
      maxDiffPixelRatio: 0.05,
    });
  });
